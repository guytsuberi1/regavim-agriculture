...
(function (global) {
  ...
  function studentReport() {
    ...
    eachCard(function (iso, c) {
      (c.students || []).forEach(function (s) {
        if (!stats[s.studentId]) stats[s.studentId] = { days: {}, work: 0, arrived: 0, hours: 0 };
        var st = stats[s.studentId];
        if (s.wentToWork) { st.days[iso] = true; st.work++; st.hours += U.num(c.hours); }
        if (s.arrived) st.arrived++;
      });
    });
    return Object.keys(stats).map(function (id) {
      var stu = Store.getById('students', id) || { name: '(נמחק)', grade: '' };
      return { name: stu.name, grade: stu.grade || '', days: Object.keys(stats[id].days).length, work: stats[id].work, arrived: stats[id].arrived, hours: stats[id].hours };
  ...
  function renderStudents() {
    ...
    gradeFilter.addEventListener('change', function () {
      U.clear(tableWrap);
      var rows = rep.filter(function (r) { return !gradeFilter.value || r.grade === gradeFilter.value; })
        .map(function (r) { return [r.name, r.grade, r.work, r.arrived, r.hours]; });
      tableWrap.appendChild(renderTable(['תלמיד', 'כיתה', 'ימי עבודה', 'ימי הגעה', 'סה"כ שעות'], rows));
    });
    var tableWrap = U.el('div');
    tableWrap.appendChild(renderTable(['תלמיד', 'כיתה', 'ימי עבודה', 'ימי הגעה', 'סה"כ שעות'], rep.map(function (r) { return [r.name, r.grade, r.work, r.arrived, r.hours]; })));
  ...
  function exportExcel() {
    ...
    XLSX.utils.book_append_sheet(wb, XLSX.utils.aoa_to_sheet([['תלמיד', 'כיתה', 'ימי עבודה', 'ימי הגעה', 'סה"כ שעות']].concat(sr.map(function (r) { return [r.name, r.grade, r.work, r.arrived, r.hours]; }))), 'תלמידים');
    var si = siteReport();
